...
def add_relative_noise(T, rot_noise, trans_noise, eps=1e-7):
  ...
  # Add rotation noise
  if rot_noise > eps:
    rot_sigma = rot_noise*br.br_transform.to_axis_angle(T)[1]
    twist = np.random.uniform(size=3, low=-rot_sigma/2., high=rot_sigma/2)
    angle = br.br_vector.norm(twist)
    axis = br.br_vector.unit(twist)
    Radd = br.br_axis_angle.to_transform(axis, angle)[:3,:3]
...
def compute_estimation_error(X, X_hat):
  ...
  rot_error = abs(br.br_transform.to_axis_angle(Rerror)[1])
...
def generate_noisy_samples(tTo, bTc, num_samples, rot_noise=0.1e-2,
  ...
  """
  samples = []
  cTb = br.br_transform.inverse(bTc)  # origin (b) wrt camera (c)
  for _ in range(num_samples):
    bTt = br.br_transform.random()   # gripper (t) wrt origin (b)
