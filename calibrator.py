...
class HandEyeCalibrator(object):
  ...
  def add_sample(self, Q, Pinv):
    ...
    """
    self.Q.append(Q)
    self.Qinv.append(br.br_transform.inverse(Q))
    self.P.append(br.br_transform.inverse(Pinv))
    ...
    if Q is not None:
      if self.num_samples == 1:
        if self.setup == Setup.Moving:
          ...
          Ap = np.dot(self.Q[0], br.br_transform.inverse(Q))
        ...
        if self.setup == Setup.Moving:
          ...
          App = np.dot(self.Q[-1], br.br_transform.inverse(Q))
    res = tuple()
    if (Ap is not None) and (App is None):
      theta_Ap = abs(br.br_transform.to_axis_angle(Ap)[1])
      t_Ap = np.linalg.norm(Ap[:3,3])
      res = (theta_Ap, t_Ap)
    elif (Ap is not None) and (App is not None):
      theta_App = abs(br.br_transform.to_axis_angle(App)[1])
      t_App = np.linalg.norm(Ap[:3,3])
      k_Ap = br.br_transform.to_axis_angle(Ap)[0]
      k_App = br.br_transform.to_axis_angle(App)[0]
  ...
  def compute_reprojection_error(self, X):
    ...
    for Ai, Bi in itertools.izip(A, B):
      ...
      rot_errors.append( br.br_transform.to_axis_angle(Rerror)[1] )
      # Metric for translation error
      tt_error = lT[:3,3] - rT[:3,3]
      bt_error = (br.br_transform.inverse(lT)[:3,3] -
                                                br.br_transform.inverse(rT)[:3,3])
