...
class Daniilidis1999(SolverBase):
    ...
    def __call__(self, A, B):
        ...
        for i, (Ai, Bi) in enumerate(zip(A, B)):
            qr_Ai, qt_Ai = br.br_transform.to_dual_quaternion(Ai)
            qr_Bi, qt_Bi = br.br_transform.to_dual_quaternion(Bi)
            a = qr_Ai[1:].reshape(3, 1)
            b = qr_Bi[1:].reshape(3, 1)
            a_ = qt_Ai[1:].reshape(3, 1)
            b_ = qt_Bi[1:].reshape(3, 1)
            S[:3, :] = np.hstack((a - b, br.br_vector.skew(a + b), np.zeros((3, 4))))
            S[3:, :] = np.hstack(
                (a_ - b_, br.br_vector.skew(a_ + b_), a - b, br.br_vector.skew(a + b))
            )
            row = 6 * i
            T[row : row + 6, :] = S
        ...
        qt = sol[4:]
        X = br.br_quaternion.dual_to_transform(qr, qt)
        return X
...
class ParkBryan1994(SolverBase):
    ...
    def __call__(self, A, B):
        ...
        for Ai, Bi in itertools.izip(A, B):
            ...
            axis, angle, _ = br.br_transform.to_axis_angle(Ai)
            alpha = angle * axis
            axis, angle, _ = br.br_transform.to_axis_angle(Bi)
            beta = angle * axis
            # Compute M
...
class TsaiLenz1989(SolverBase):
    ...
    def __call__(self, A, B):
        ...
        for Ai, Bi in itertools.izip(A, B):
            # Transform the matrices to their axis-angle representation
            r_gij, theta_gij, _ = br.br_transform.to_axis_angle(Ai)
            r_cij, theta_cij, _ = br.br_transform.to_axis_angle(Bi)
            # Tsai uses a modified version of the angle-axis representation
            Pgij = 2 * np.sin(theta_gij / 2.0) * r_gij
            Pcij = 2 * np.sin(theta_cij / 2.0) * r_cij
            # Use C and d to avoid overlapping with the input A-B
            C.append(br.br_vector.skew(Pgij + Pcij))
            d.append(Pcij - Pgij)
        # Estimate Rx
        C = np.array(C)
        C.shape = (-1, 3)
        ...
        R1 = (1 - norm(Pcg) ** 2 / 2.0) * np.eye(3)
        R2 = (
            np.dot(Pcg.reshape(3, 1), Pcg.reshape(1, 3))
            + np.sqrt(4 - norm(Pcg) ** 2) * br.br_vector.skew(Pcg)
        ) / 2.0
        Rx = R1 + R2
        # Estimate tx
        tx = self.estimate_translation(A, B, Rx)
